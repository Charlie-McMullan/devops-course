...
resource "aws_iam_policy" "task_execution_role_policy" {
  ...
  path        = "/"
  description = "Allow ECS to retreive images and add to logs"
  policy      = file("./templates/ecs/task-execution-role-policy.json")
}

resource "aws_iam_role" "task_excecution_role" {
  name               = "${local.prefix}-task-execution-role"
  assume_role_policy = file("./templates/ecs/task-assume-role-policy.json")
}

resource "aws_iam_role_policy_attachment" "task_excecution_role" {
  role       = aws_iam_role.task_excecution_role.name
...
resource "aws_ecs_task_definition" "api" {
  ...
  cpu                      = 256
  memory                   = 512
  execution_role_arn       = aws_iam_role.task_excecution_role.arn
...
resource "aws_ecs_service" "api" {
  ...
  enable_execute_command = aws_iam_role.task_excecution_role
